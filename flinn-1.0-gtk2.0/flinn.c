/*
Flinn - v0.95 / 2007-02-04 (jonas.jensen@gmail.com)
This is based on Flynn v0.94 (adam@gimp.org)
Flinn is a modified Flynn for rotated images, perl run_game script, analog joystick and hotkeys for inserting the -ror switch to xmame
*/
...
void save_state(void) {
	...
	fprintf(fp, "%d %d %d %d",flynn_state.ox,flynn_state.oy,flynn_state.gx,flynn_state.gy);	
	fclose(fp);
}
...
void load_state(void) {
	...
	fscanf(fp, "%d %d %d %d",&flynn_state.ox,&flynn_state.oy,&flynn_state.gx,&flynn_state.gy);	
	fclose(fp);
}
...
gint key_press(GtkWidget *widget,GdkEvent  *event) {
	GdkEventKey    *kevent;
	...
	switch (kevent->keyval) {
		...
		case GDK_Down:
			if (isrotated()) { 
				if (flynn_state.gx > 0) flynn_state.gx--;
			}
			else { 
				if (flynn_state.gy < (num_games+COLUMNS-1)/COLUMNS -1) flynn_state.gy++; 
			}
			break;
		case GDK_Up:			
			if (isrotated()) { 
				if (flynn_state.gx < COLUMNS-1) flynn_state.gx++;
			}
			else { 
				if (flynn_state.gy > 0) flynn_state.gy--; 
			}
			break;
		case GDK_Left:			
			if (isrotated()) { 
...
void show(void) {
	gdk_draw_rgb_image (drawing_area->window, drawing_area->style->white_gc,	0, 0, WWIDTH, WHEIGHT, /* GDK_RGB_DITHER_NORMAL, */ GDK_RGB_DITHER_NONE,/* GDK_RGB_DITHER_MAX, */ (guchar*)disp, WWIDTH*3);
	gdk_flush();
}


void put_rgb(unsigned char *data, int data_w, int data_h, int buf_w, int buf_h, int x, int y) {
	int i,j;
	
	if (x <= -data_w || y <= -data_h || x >= buf_w || y >= buf_h) {
		return;
	}
	
	if (x>=0 && y>=0 && x+data_w<buf_w && y+data_h<buf_h) {
		/*for (j=0; j<data_h; j++) {
		memcpy(&disp[3*(buf_w*(y+j) + x)],&data[3*data_w * j],3*data_w);
		}*/
		for (j=0; j<data_h; j++) {
			memcpy(&disp[3*(buf_w*(y+j) + x)],&data[3*data_w * j],3*data_w);
		}		
...
int iterate(void* whatever) {
	...
	fabs(23.0F + 64.0F*sin(((double)(i+flynn_state.oy/3.0F))/20.0F + s1o) + 58.0F*cos(1.0 + ((double)(i+flynn_state.oy/2.0F))/155.0F + s2o)),3*WWIDTH);
	...
	for (i=0; i<num_games; i++) {
		...
		if (i%COLUMNS == flynn_state.gx && i/COLUMNS == flynn_state.gy) {
			selected = i;
			...
			dx += constrain * (sin(s4o + 0.8F*((double)(i%COLUMNS + i/COLUMNS)) ) * JIGGLE);
			dy += constrain * (cos(s5o + 0.8F*((double)(i%COLUMNS - i/COLUMNS)) ) * JIGGLE);
			...
			put_rgb(game[i].ppm_data,PPM_WIDTH,PPM_HEIGHT,WWIDTH,WHEIGHT,dx,dy);
			//put_rgb2(game[i].ppm_data,PPM_WIDTH,PPM_HEIGHT,WWIDTH,WHEIGHT,dx,dy);
		}
	}
	...
	if (selected != -1) {
		...
		for (i=0; i<NUM_SPARK; i++) {
			put_rgb(game[selected].ppm_data, PPM_WIDTH, PPM_HEIGHT, WWIDTH, WHEIGHT,	sx + (spark_out * cos(s3o + i*2.0F*M_PI/(double)NUM_SPARK)), sy + (spark_out * sin(s3o + i*2.0F*M_PI/(double)NUM_SPARK)));
		}		
		/* draw selected icon on top */
		put_rgb(game[selected].ppm_data, PPM_WIDTH, PPM_HEIGHT,WWIDTH, WHEIGHT, sx, sy);
	}
...
int main(int argc, char *argv[]) {
	getcwd(cwd, 999);
	arg0 = argv[0];
	arg1 = (argc > 2) ? argv[1] : NULL;
	workdir = (argc > 2) ? argv[2] : NULL;
	if (argc == 2) { 
		arg1 = NULL;
		workdir = argv[1];
	}
